...
> 版本：1.2 草案 | 日期：2026-05-27 | 关联：[LAYERED-ARCHITECTURE.md](LAYERED-ARCHITECTURE.md) v2.5、[OUTBOUND-DIALOG-PLAN.md](OUTBOUND-DIALOG-PLAN.md) v1.2、[HORIZONTAL-SCALING.md](HORIZONTAL-SCALING.md)
>
...
- 单机部署示例
- 多节点部署示例（含 Redis store 模板代码 + nodeAddressMap），**`sip.server.external-ip: <VIP>` 标注为必填项**——多节点下节点 IP 不可达，必须填 VIP 地址，否则设备回包绕过 VIP 导致源 IP 哈希失效（详见 [HORIZONTAL-SCALING.md](HORIZONTAL-SCALING.md)）
- 错误码契约
