...
| 附录 A.3（前端设备控制协议） | 规范性 | 8 字节二进制 PTZ 指令编码（§A.3.1 格式 + §A.3.2-7 PTZ/FI/Preset/Cruise/Scan/Aux 6 类） | ✅ v1.6.0 全量交付 | `gb28181-common/.../instruction/PTZInstructionFormat` + `PTZInstructionBuilder` + 6 个 enum；`ServerCommandSender.deviceControl{PtzCmd,FI,Preset,Cruise,Scan,Auxiliary}` 6 类高层 API；29 单元测试 + 6 端到端 FlowTest（详见 [FRONT-END-CONTROL-PROTOCOL-PLAN.md](FRONT-END-CONTROL-PROTOCOL-PLAN.md)） |
