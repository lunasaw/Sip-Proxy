...
| 节号 | 命令 | 状态 | 落点 |
|---|---|---|---|
| A.2.3.1.2 | 摄像机云台控制 PTZCmd | ✅ | `ControlListener.onPtz`；`<PTZCmd>` 8 字节 hex 编码遵循 §A.3.1-7（落点：`PTZInstructionBuilder` + `PTZInstructionFormat`）；server 端高层 API：`ServerCommandSender.deviceControl{PtzCmd,FI,Preset,Cruise,Scan,Auxiliary}` 6 类（v1.6.0） |
