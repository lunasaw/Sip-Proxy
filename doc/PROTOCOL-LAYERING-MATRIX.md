...
> 版本：v1.6.0 | 日期：2026-05-25 | 状态：基于代码事实生成
>
...
| A.2.3.1.6 | 报警复位 AlarmCmd | ✅ | `ControlListener.onAlarmReset` | ❌ 无 |
...
| 节号 | 命令 | 状态 | 落点 | FlowTest |
|---|---|---|---|---|
| A.2.3.2.2 | 基本参数 BasicParam | ✅ | `ConfigListener.onBasicParamConfig` | ❌ 无 |
| A.2.3.2.3 | SVAC 编码 | ✅ | `ConfigListener.onSvacEncodeConfig` | ❌ 无 |
| A.2.3.2.4 | SVAC 解码 | ✅ | `ConfigListener.onSvacDecodeConfig` | ❌ 无 |
| A.2.3.2.5 | 视频参数属性 | ✅ | `ConfigListener.onVideoParamAttributeConfig` | ❌ 无 |
| A.2.3.2.6 | 录像计划 | ✅ | `ConfigListener.onVideoRecordPlanConfig` | ❌ 无 |
...
| A.2.3.2.8 | 视频画面遮挡 PictureMask | ✅ | `ConfigListener.onPictureMaskConfig` | ❌ 无 |
| A.2.3.2.9 | 画面翻转 FrameMirror | ✅ | `ConfigListener.onFrameMirrorConfig` | ❌ 无 |
...
| —（GB28181-2016 遗留） | 视频参数范围 VideoParamOpt | ✅ 超出标准 | `ConfigListener.onVideoParamOptConfig`；2022 标准只在 §A.2.6.9 应答中允许携带，下发不在 §A.2.3.2.x 序列内，保留以兼容 2016 设备 | ❌ 无 |
...
| A.2.4.5 | RecordInfo | ✅ | `QueryListener.onRecordInfoQuery`（仅核心字段，扩展过滤字段 Type/RecorderID 等未结构化暴露，业务方可在 `DeviceRecordQuery` 上拿到原始字段） | ❌ 无 |
...
| A.2.5.4 | MediaStatus | ✅ | `DeviceNotifyListener.onMediaStatus` | ❌ 无 |
| A.2.5.5 | Broadcast（语音广播通知） | ✅ | Client：`NotifyListener.onBroadcastNotify`；Server：`ServerCommandSender.deviceBroadcast` 出站 | ❌ 无 |
...
| A.2.6.7 | RecordInfo 应答 | ✅ | `DeviceResponseListener.onRecordInfoResponse` | ❌ 无 |
| A.2.6.8 | DeviceConfig 应答 | ✅ | `DeviceResponseListener.onConfigResponse`（CMD_TYPE=DeviceConfig，仅结果码） | ❌ 无 |
...
| §9.7 / §9.8 / §9.9 | 历史检索 / 回放 / 下载（INFO + MANSRTSP） | ✅ | `ClientInfoEvent`（payload=`String` 或 `ManSrtspRequest`）；MANSRTSP PLAY/PAUSE/TEARDOWN/SCALE 已支持 | ❌ 无 |
...
| §9.12.1 | 语音广播 | ✅ | Client：`NotifyListener.onBroadcastNotify` + `ClientInviteEvent`（媒体流建立） | ❌ 无 |
...
> |---|---|---|---|---|---|---|
> | (A) §A.2.3.1 设备控制 | 13 | 12 | — | 1（A.2.3.1.6 AlarmCmd）| — | 92.3% |
> | (A) §A.2.3.2 设备配置（含 1 兼容）| 12 | 4 | — | 8 | — | 33.3% |
> | (A) §A.2.4 查询 | 13 | 9 | 1（MobilePosition 仅路由保护）| 3 | — | 69.2% |
> | (A) §A.2.5 通知 | 8 | 6（含 v1.6.1 新增 KeepaliveFlowTest）| — | 2（MediaStatus/Broadcast）| — | 75.0% |
> | (A) §A.2.6 应答 | 15 | 10 | — | 2（RecordInfo/DeviceConfig）| 3 | 83.3%（10/12 可测）|
> | (B) §9 流程级 | 9 | 5 | 1（§9.10 校时间接）| 3（§9.7-9 INFO + §9.12.1 + §9.12.2）| — | 55.6% |
> | **总计** | **70** | **46** | **2** | **19** | **3** | **68.7%（46/67 可测）** |
>
> 缺口集中在 **§A.2.3.2 设备配置（8/12 缺）** 和 **§A.2.6 应答类**。建议在 v1.6.x 周期内按 P0/P1 分批补齐，优先补：BasicParam/SVAC 配置、PictureMask/FrameMirror、VideoRecordPlan、RecordInfo Query+Response、MediaStatus、§9.7-9 INFO MANSRTSP、Broadcast。
>
