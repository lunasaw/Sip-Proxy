...
>
> **实施结果**：本方案 Stage 1-4 已于 2026-05-25 全量交付，详见 [PROTOCOL-LAYERING-MATRIX.md §九 v1.6.0 条目](../../architecture/PROTOCOL-LAYERING-MATRIX.md)。
...
>
> 关联：[GBT-28181-2022.md §A.3](../../protocol/2022/GBT-28181-2022.md)、[PROTOCOL-LAYERING-MATRIX.md](../../architecture/PROTOCOL-LAYERING-MATRIX.md)、[LISTENER-LAYERED-DESIGN.md](../../architecture/LISTENER-LAYERED-DESIGN.md)
>
...
**协议分层定位（与 [PROTOCOL-LAYERING-MATRIX.md](../../architecture/PROTOCOL-LAYERING-MATRIX.md) 三层映射对应）**：
...
**§A.3 整体属于 [LISTENER-LAYERED-DESIGN.md](../../architecture/LISTENER-LAYERED-DESIGN.md) 的 L0 协议解析层之下的「编码 utility 层」**，不引入新的 listener 接口或 dispatcher key。本方案仅梳理"如何把 8 字节二进制规则正确序列化进 `<PTZCmd>` 字段"。
...
2. **零 JUnit 测试**：`gb28181-common/src/test/.../control/instruction/` 目录不存在，4 个 `*Validation`/`*Demo` 文件用 `main()` 做断言，违反 [LISTENER-LAYERED-DESIGN.md](../../architecture/LISTENER-LAYERED-DESIGN.md) 与 voglander 项目「Manager 类必须用 `@SpringBootTest` 集成测试」的规范，��无法纳入 JaCoCo 80% 覆盖率门槛。
3. **过度设计**：`PTZInstructionCrypto` 给二进制指令加 AES-GCM 加密 —— GBT-28181-2022 §A.3 全文未要求加密，§8.2 数据加密走的是 IPSec/TLS 传输层方案（详见 [PROTOCOL-LAYERING-MATRIX.md §0(C)](../../architecture/PROTOCOL-LAYERING-MATRIX.md)），与 8 字节 payload 无关。
...
**更新** [PROTOCOL-LAYERING-MATRIX.md](../../architecture/PROTOCOL-LAYERING-MATRIX.md)：
...
按 [PROTOCOL-LAYERING-MATRIX.md](../../architecture/PROTOCOL-LAYERING-MATRIX.md) §十「维护承诺」，本方案严格遵守：
...
v1.5.6 完成了 GBT-28181-2022 附录 A **命令集** 100% 覆盖（详见 [PROTOCOL-LAYERING-MATRIX.md §0(A)](../../architecture/PROTOCOL-LAYERING-MATRIX.md)），但 §A.3 **二进制指令编码层** 不在该范围 —— 它是 §A.2.3.1.2 命令的 payload 子规则，与"协议命令是否覆盖"是正交的两个维度。
...
A：§A.3 是编码细节、不是协议契约。业务方接收 `<PTZCmd>` hex 时（client 侧）通过 `ControlListener.onPtz(cmd)` 拿到的就是 `DeviceControlPtz.ptzCmd` 字符串，需要解码时业务方自行调 `PTZInstructionFormat.fromHexString(hex)` 即可。提升到 listener 会让 sip-proxy 替业务方做语义解释，违反「协议层不上诠释」的 [LISTENER-LAYERED-DESIGN.md](../../architecture/LISTENER-LAYERED-DESIGN.md) 约定。
...
| `doc/architecture/PROTOCOL-LAYERING-MATRIX.md` | +20 行 | §0(D) + §九版本日志 |
