...
/**
 * SIP事务上下文管理器
 * 通过ThreadLocal管理SIP事务的Call-ID，确保请求-响应-ACK整个链路使用同一个Call-ID
 * <p>
 * 支持两种模式：
 * 1. 显式模式：通过setRequestEvent直接设置原始请求事件
 * 2. 隐式模式：通过setCallId设置Call-ID，在消息处理链路中自动传递
 *
 * @author luna
 * @date 2024/08/11
 */
...
public class SipTransactionContext {
    ...
    public static void setRequestEvent(RequestEvent requestEvent) {
        if (requestEvent != null) {
            ...
            log.debug("设置SIP事务上下文（显式模式）: callId={}", callId);
        }
    }
    ...
    public static void setCallId(String callId) {
        if (callId != null && !callId.trim().isEmpty()) {
            ...
            log.debug("设置SIP事务上下文（隐式模式）: callId={}", callId);
        }
    }
