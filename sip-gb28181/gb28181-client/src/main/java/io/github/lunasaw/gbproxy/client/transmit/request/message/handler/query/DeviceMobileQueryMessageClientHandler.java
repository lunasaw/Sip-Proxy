...
/**
 * 设备移动位置查询消息处理器（cmdType=MobilePosition，rootType=Query）。
 *
 * <p>v1.5.0 改造：只发布 {@link ClientQueryEvent}，回包由 {@code ClientListenerAdapter}
 * 路由到 {@code QueryListener.onMobilePositionQuery}。
 *
 * @author luna
 */
@Component
...
public class DeviceMobileQueryMessageClientHandler extends MessageClientHandlerAbstract {
    ...
    public void handForEvt(RequestEvent event) {
        try {
            ...
            MobilePositionQuery query = parseXml(MobilePositionQuery.class);
            if (query == null) {
                log.warn("MobilePosition 查询解析失败");
                return;
            }
            publisher.publishEvent(new ClientQueryEvent(this,
                    deviceSession.getUserId(), deviceSession.getSipId(), query));
        } catch (Exception e) {
            log.error("处理设备移动位置查询时发生异常: event = {}", event, e);
        }
    }
