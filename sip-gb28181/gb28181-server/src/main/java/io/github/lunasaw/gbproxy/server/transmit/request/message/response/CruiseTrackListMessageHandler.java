...
/**
 * GB28181-2022 §9.5 / A.2.6.13 巡航轨迹列表查询应答 处理器
 *
 * @author luna
 */
@Component
@Slf4j
public class CruiseTrackListMessageHandler extends MessageServerHandlerAbstract {

    public static final String CMD_TYPE = CmdTypeEnum.CRUISE_TRACK_LIST_QUERY.getType();

    public CruiseTrackListMessageHandler(ApplicationEventPublisher publisher, ServerDeviceSupplier serverDeviceSupplier) {
    ...
    public void handForEvt(RequestEvent event) {
        ...
        CruiseTrackListResponse response = parseXml(CruiseTrackListResponse.class);
        if (response == null) {
            log.warn("解析巡航轨迹列表查询应答失败: deviceId={}", userId);
            return;
        }
